...
function App() {
  ...
  const refreshProjectState = async () => {
    if (!project) return;
    const result = await request<{ state: Record<string, unknown> }>(`/api/projects/${project.project_id}`, token);
    setProject((current) => current ? { ...current, state: result.state } : current);
    const artifacts = (result.state.artifacts ?? {}) as Record<string, unknown>;
    setExportAvailable(Boolean(artifacts.final_video));
  };
  ...
  const upload = async (file: File) => {
    ...
    try {
      ...
      const artifacts = (result.project.state.artifacts ?? {}) as Record<string, unknown>;
      setExportAvailable(Boolean(artifacts.final_video));
  ...
  const retry = async () => {
    ...
    try {
      ...
      setJob(result.job); await watchJob(result.job.job_id); await refreshProjectState(); setMessage("Retry finished.");
  ...
  const rebuild = async () => {
    ...
    try {
      ...
      setJob(result.job); await watchJob(result.job.job_id); await refreshProjectState(); setMessage("Rebuild finished.");
  ...
  const downloadExport = async () => {
    if (!project) return;
    try {
      const response = await fetch(`/api/projects/${project.project_id}/artifact/final_video`, {
      ...
      anchor.download = `${project.filename.replace(/\.[^/.]+$/, "")}_export.mp4`;
